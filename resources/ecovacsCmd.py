...
try:
    ...
    from deebot_client.events import (
        AvailabilityEvent, BatteryEvent, CleanLogEvent, ErrorEvent,
        FanSpeedEvent, LifeSpanEvent, NetworkInfoEvent, StateEvent,
        StatsEvent, TotalStatsEvent,
    )
    ...
    from deebot_client.events.water_info import (
        WaterAmount, WaterAmountEvent, WaterCustomAmountEvent,
        WaterSweepTypeEvent, MopAttachedEvent, SweepType,
    )
...
_REFRESH_COMMANDS = [
    GetBattery(), GetFanSpeed(), GetStats(), GetTotalStats(), GetError(),
    GetCleanLogs(), GetNetInfo(), GetWorkMode(), GetWaterInfo(),
    GetMopAutoWashFrequency(), GetChargeState(), GetVolume(),
]


def _make_fire(did, session, callback_url, apikey, loop):
    def fire(data):
        data["did"] = did
...
async def _post_callback(session, callback_url, apikey, data):
    ...
    try:
        async with session.post(
            ...
            if resp.status != 200 or body.strip() != "OK":
                ...
                logger.debug("CallbackOK[%s] type=%s", data.get("did"), data.get("type"))
    except Exception as exc:
        logger.error("Callback HTTP échoué : %s", exc)


def subscribe_all_events(device, fire):
    async def on_availability(e): fire({"type": "availability", "available": e.available})
    device.events.subscribe(AvailabilityEvent, on_availability)

    async def on_battery(e): fire({"type": "battery", "value": e.value})
    device.events.subscribe(BatteryEvent, on_battery)

    async def on_state(e): fire({"type": "state", "state": STATE_LABEL.get(e.state, str(e.state))})
    device.events.subscribe(StateEvent, on_state)

    async def on_fan_speed(e): fire({"type": "fan_speed", "speed": e.speed.name})
    device.events.subscribe(FanSpeedEvent, on_fan_speed)

    async def on_work_mode(e): fire({"type": "work_mode", "mode": e.mode.name})
    device.events.subscribe(WorkModeEvent, on_work_mode)

    async def on_water_amount(e): fire({"type": "water_amount", "amount": e.value.name})
    device.events.subscribe(WaterAmountEvent, on_water_amount)

    async def on_water_custom(e): fire({"type": "water_custom_amount", "value": e.value})
    device.events.subscribe(WaterCustomAmountEvent, on_water_custom)

    async def on_sweep_type(e): fire({"type": "sweep_type", "sweep_type": e.value.name})
    device.events.subscribe(WaterSweepTypeEvent, on_sweep_type)

    async def on_mop_attached(e): fire({"type": "mop_attached", "attached": e.value})
    device.events.subscribe(MopAttachedEvent, on_mop_attached)

    async def on_stats(e): fire({"type": "stats", "area": e.area, "duration": e.time, "mode": e.type})
    device.events.subscribe(StatsEvent, on_stats)

    async def on_total_stats(e): fire({"type": "total_stats", "area": e.area, "time": e.time, "cleanings": e.cleanings})
    device.events.subscribe(TotalStatsEvent, on_total_stats)

    async def on_error(e): fire({"type": "error", "code": e.code, "description": e.description or ""})
    device.events.subscribe(ErrorEvent, on_error)

    async def on_lifespan(e):
        fire({"type": "lifespan", "component": e.type.name, "value": e.type.value,
              "percent": e.percent, "remaining": e.remaining})
    device.events.subscribe(LifeSpanEvent, on_lifespan)

    async def on_network(e): fire({"type": "network", "ip": e.ip, "ssid": e.ssid, "rssi": e.rssi, "mac": e.mac})
    device.events.subscribe(NetworkInfoEvent, on_network)

    async def on_clean_logs(e):
        entries = [{"timestamp": x.timestamp, "area": x.area, "duration": x.duration,
                    "type": x.type, "stop_reason": str(x.stop_reason)} for x in e.logs]
        fire({"type": "clean_logs", "entries": entries})
    device.events.subscribe(CleanLogEvent, on_clean_logs)

    async def on_volume(e): fire({"type": "volume", "value": e.volume})
    device.events.subscribe(VolumeEvent, on_volume)

    async def on_station_state(e):
        station_labels = {
    ...
    async def on_auto_empty(e):
        fire({"type": "auto_empty", "enabled": e.enabled,
              "frequency": e.frequency.value if e.frequency else ""})
    device.events.subscribe(AutoEmptyEvent, on_auto_empty)

    async def on_mop_wash(e): fire({"type": "mop_wash_frequency", "value": e.value})
    device.events.subscribe(MopAutoWashFrequencyEvent, on_mop_wash)


async def refresh_device(device):
    cmds = list(_REFRESH_COMMANDS)
    # Utiliser uniquement les lifespans supportés par le modèle
...
class SocketServer(threading.Thread):
    def __init__(self, port, apikey, devices, device_list, loop, fire_map):
    ...
    def run(self):
        ...
        try:
            ...
            logger.critical("FATAL: Impossible de binder le port %d : %s", self._port, exc)
            os._exit(1)
    ...
    def _handle(self, conn):
        try:
            ...
            if msg.get("apikey") != self._apikey:
                logger.warning("Socket : clé API invalide")
                return
            ...
            if device is None:
                logger.warning("Socket : did=%s inconnu", did)
                return
            asyncio.run_coroutine_threadsafe(
            ...
            logger.error("Socket : erreur – %s", exc)
        finally:
            conn.close()

    async def _dispatch(self, device, action, msg):
        did = device.device_info["did"]
        fire = self._fire_map.get(did)
        try:
            match action:
                case "clean":
                    ca_map = {
                ...
                case "fan_speed":
                    ...
                    cmd = (device.capabilities.fan_speed.set(fan)
                           if device.capabilities.fan_speed else SetFanSpeed(fan))
                    await device.execute_command(cmd)
                    # Refresh différé pour confirmer la valeur
                    await asyncio.sleep(2)
                    await device.execute_command(GetFanSpeed())
                ...
                case "work_mode":
                    ...
                    cmd = (device.capabilities.clean.work_mode.set(wm)
                           if device.capabilities.clean.work_mode else SetWorkMode(wm))
                    await device.execute_command(cmd)
                    # Refresh différé pour confirmer la valeur
                    await asyncio.sleep(2)
                    await device.execute_command(GetWorkMode())
                ...
                case "sweep_type":
                    sweep_map = {"standard": SweepType.STANDARD, "deep": SweepType.DEEP}
                ...
                case "station_action":
                    ...
                    val = msg.get("value", "empty_dustbin").strip().lower()
                    sa = action_map.get(val)
                    if sa is None:
                        logger.warning("Socket : station_action invalide '%s'", val)
                        return
                ...
                case _:
                    logger.warning("Socket : action inconnue '%s'", action)
...
async def main():
    ...
    parser.add_argument("--socketport", default=55009, type=int)
    ...
    async with aiohttp.ClientSession() as http_session:
        ...
        rest_config = create_rest_config(
            http_session, device_id=device_id, alpha_2_country=args.country.upper(),
        )
        ...
        mqtt_devices = all_devices.mqtt
        if not mqtt_devices:
            logger.warning("Aucun robot MQTT compatible trouvé.")
        ...
        # ── Hook MQTT brut ────────────────────────────────────────────────────
        _did_fire_map = dict(fire_map)
        original_handle = mqtt_client._handle_message

        def _extra_handler(message):
            try:
                ...
                if cmd_name == "getWashInfo":
                    ...
                    fire_fn({"type": "fan_speed_raw", "raw": raw, "speed": speed_map.get(raw, str(raw))})
                    ...
                    # Mapper depuis state_raw en priorité
                    state_map = {0: "idle", 1: "emptying", 2: "drying", 3: "washing", 4: "washing"}
                    mapped = state_map.get(state_raw, "idle")

                    # Codes erreur bacs
                    dirty_water_full  = 1 if any(c in errors for c in [302, 305, 318, 323]) else 0
                    clean_water_empty = 1 if any(c in errors for c in [301, 303, 315, 322]) else 0
                    ...
                    fire_fn({"type":              "station_state_raw",
                             ...
                             "clean_water_empty": clean_water_empty,
                    })

                elif cmd_name == "getVolume":
                    fire_fn({"type": "volume", "value": body_data.get("volume", 0)})

                elif cmd_name == "onWorkProgressReport":
                    fire_fn({"type":  "work_progress",
                    ...
                    for item in items:
                        ...
                        if ltype:
                            fire_fn({"type": "lifespan", "component": ltype.upper(),
                                     "value": ltype, "percent": percent, "remaining": left})
